...
export const loginObject=z.object({
  ...
  export const AddBlogObject = z.object({
    ...
    images:z.instanceof(FileList, { message: 'image is required' }).refine((file)=>file.length >=1,{message:"image is required"}).refine((filetype)=>ACCEPTED_IMAGE_TYPES.includes(filetype[0]?.type),{message:"Not a valid image"}).refine((file) => file[0]?.size <= 1000000, {message:`Max image size is 1mb.`}),
  })
